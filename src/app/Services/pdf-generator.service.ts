...
export class PdfGeneratorService {
  private readonly colors = {
    ...
    high: {
      bg: [253, 126, 20] as [number, number, number],
      text: [255, 255, 255] as [number, number, number],
    },
    medium: {
    ...
    low: {
      bg: [40, 167, 69] as [number, number, number],
      text: [255, 255, 255] as [number, number, number],
    },
  ...
  async generateProjectReportPDFAlternative(project: any): Promise<void> {
    ...
    const tocMap: Record<string, number> = {};
    let yPosition = 20;
    let pageCounter = 0; // Track numbered
    ...
    // 8. Apply footers to all pages except the cover page
    this.addFooterExceptCoverPage(doc);
  ...
  private async addCoverPage(doc: jsPDF, project: any): Promise<number> {
    ...
    try {
      ...
      doc.setFontSize(8);
      doc.text('LOGO', logoX + logoMaxWidth / 2, logoY + logoHeight / 2, {
        align: 'center',
      });
    }
    ...
    const decoY =
      contentBottomY + (footerY - contentBottomY) / 2 - decoSize / 2;
  ...
  private addTableOfContents(
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.text('TABLE OF CONTENTS', pageWidth / 2, yPosition, {
      align: 'center',
    });
    ...
    tocItems.forEach((item) => {
      ...
      const dotsEnd = rightX - pageNumberWidth - 3;
  ...
  private addExecutiveSummary(
    ...
    if (critical > 0) {
      distributionTexts.push(
        `${critical} critical issue${
          critical > 1 ? 's' : ''
        } requiring immediate remediation to prevent potential system failures or security breaches.`
      );
    }
    ...
    if (high > 0) {
      distributionTexts.push(
        `${high} high-priority issue${
          high > 1 ? 's' : ''
        } that could significantly impact system reliability and user experience.`
      );
    }
    ...
    if (medium > 0) {
      distributionTexts.push(
        `${medium} medium-priority issue${
          medium > 1 ? 's' : ''
        } affecting system efficiency and requiring planned resolution.`
      );
    }
    ...
    if (low > 0) {
      distributionTexts.push(
        `${low} low-priority issue${
          low > 1 ? 's' : ''
        } consisting of minor inconsistencies and cosmetic concerns.`
      );
    }
    ...
    if (totalFindings === 0) {
      distributionTexts.push(
        `No significant issues were identified during the assessment, indicating a well-maintained and stable system.`
      );
    }
    ...
    if (distributionTexts.length > 0) {
      findingsNarrative = `The assessment revealed ${distributionTexts.join(
      ...
      if (critical > 0 || high > 0) {
        findingsNarrative +=
          ' Priority should be given to addressing the most severe issues to maintain system integrity and prevent potential disruptions.';
      } else if (medium > 0 || low > 0) {
        findingsNarrative +=
          ' These findings present opportunities for system optimization and quality enhancement.';
      }
    }
  ...
  private addSeveritySummary(
    ...
    const totalContentHeight =
      headerHeight + cardHeight + summaryBarHeight + spacing + 35;

    // Check if we need a new page
    if (yPosition + totalContentHeight > 280) {
      // Assuming page height ~280
      doc.addPage();
      yPosition = 20;
    }
    ...
    const startX =
      20 +
      (170 -
        (Object.keys(severityCounts).length * cardWidth +
          (Object.keys(severityCounts).length - 1) * cardSpacing)) /
        2;
    let xPosition = startX;

    Object.entries(severityCounts).forEach(([severity, count]) => {
      const color = this.colors[
        severity.toLowerCase() as keyof typeof this.colors
      ] as any;
      const percentage =
      ...
      doc.setFontSize(24);
      doc.setFont('helvetica', 'bold');
      doc.text(count.toString(), xPosition + cardWidth / 2, yPosition + 28, {
        align: 'center',
      });
      ...
      doc.setFontSize(7);
      doc.setFont('helvetica', 'normal');
      doc.text(`${percentage}%`, xPosition + cardWidth / 2, yPosition + 36, {
        align: 'center',
      });
      ...
      doc.setFontSize(6);
      doc.setFont('helvetica', 'normal');
      doc.text('FINDINGS', xPosition + cardWidth / 2, yPosition + 44, {
        align: 'center',
      });
    ...
    // summary statistics below cards
  ...
  private calculateAverageRiskLevel(severityCounts: any): {
    level: string;
    score: number;
  } {
  ...
  private async addSingleFindingWithHeaders(
    ...
    const severityColor =
      'bg' in colorEntry ? (colorEntry.bg as number[]) : [0, 0, 0];
    ...
    if (finding.description) {
      ...
      const { text: descText, images } = await this.parseEditorJSContent(
        finding.description,
        true
      );

      // Process text with headers and images
      y = await this.processTextWithHeadersAndImages(
        doc,
        descText,
        images,
        startX,
  ...
  private async processTextWithHeadersAndImages(
    ...
    for (let i = 0; i < textParts.length; i++) {
      ...
      if (part) {
        ...
        const estimatedTextHeight = this.estimateTextHeight(
          doc,
          part,
          maxWidth
        );
        ...
        currentY = await this.processTextWithHeaders(
          doc,
          part,
          startX,
          currentY,
  ...
  private async parseEditorJSContent(
    description: any,
    collectImages: boolean = false
  ): Promise<{
    text: string;
    ...
    for (const block of description.blocks) {
      ...
      switch (type) {
        ...
        case 'Header':
          ...
          contentLines.push(
            `[HEADER:${level}:${fontSize}]${headerText}[/HEADER]`
          );
          break;
        ...
        case 'List': {
          ...
          listItems.forEach((item) => {
            ...
            const line =
              item.trim().startsWith('•') || item.trim().match(/^\d+\./)
                ? '        ' + item
                : '        • ' + item;
        ...
        case 'ImageTool':
          if (collectImages) {
            ...
            if (imageResult.base64) {
              const imageCaption =
                data.caption || data.file?.caption || `Image ${imageCount + 1}`;
              images.push({
      ...
      return new Promise((resolve) => {
        img.onload = () => {
          ...
          doc.setTextColor(
            ...
            this.colors.secondary[1],
            this.colors.secondary[2]
          );
          doc.text(image.caption, 25 + maxWidth / 2, yPosition, {
            align: 'center',
          });
  ...
  private async handleImageBlock(
    data: any,
    contentLines: string[]
  ): Promise<void> {
    if (!data) {
  ...
  private addFooterExceptCoverPage(doc: jsPDF): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.text('Sibasi Limited QA Report', 20, pageHeight - 10, {
        align: 'left',
      });
    }
  }
